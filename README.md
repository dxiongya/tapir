...
<p align="center">
  <sub>
  ↑ <strong>这是一整台 Mac 屏幕的「一天点击热力图」。</strong>每个亮点 = 当天某一次鼠标点击的位置；颜色越亮，那块区域被点得越多。
  <br>
  顶部那条横向亮带是浏览器 tab / 菜单栏；左下角几颗高亮是 Dock 常用图标；中间散开的小点是各 App 窗口的内容区。
  <br>
  整张图<strong>完全去隐私</strong>：没有窗口标题、没有按钮文本、没有 URL —— 只有匿名坐标 + 总点击数 + 高频 App / 活跃时段。
  </sub>
</p>
